...
# $refG = '/u/home/j/jxzhai/SCRATCH/Casava/reference/EACMV-UG-CMT3.fasta';
...
$BSMAP_PM = '-p 2 -w 1 -n 0 -v 0.04 '; # allow 4% mismatch
...
while (defined(my $seqfile = readdir(DIR))) {
    unless ($seqfile =~ /(.*)_1\.fastq\.gz/) {next;}
    # unless ($seqfile =~ /(.*)\_R1\.fastq/) {next;}
    $lib_name = $1;
    $seqfile2 = $seqfile;
    $seqfile2 =~ s/\_1/\_2/;
...
$command =
"
bsmap -a $seqdir$seqfile -b $seqdir$seqfile2 -d $refG -o $lib_name\.bam $BSMAP_PM
...
";
    print "$command\n";
    open(SH, ">PE\_$lib_name\_BSMAP.sh");
    print SH "$PM";
    print SH "$command\n";
    system "qsub PE\_$lib_name\_BSMAP.sh\n";
    close SH;
    system "rm PE\_$lib_name\_BSMAP.sh";
}